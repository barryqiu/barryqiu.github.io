...
从上面的说明结合前面redis go的代码可以看出针对 redis 协议中的每一行数据返回，都会为其创建一个 slice（Go 里面的变长数组） ，那么这里的内存分配大就很好理解了，以评论属性为例，每个评论属性大约有35个字段，每次请求平均会拉取20条评论，而作为hash结构其 key 和 value 会分成两行来拉取，那么简单计算一下， **每个request仅仅在评论属性的redis读取部分就需要创建 35*2*20=1400个小对象**，当QPS达到5000时，假设所有请求都是从redis 获取那么每秒将会创建 7000000 个小对象，这些小对象用完就废弃了，都需要交给 GC 来回收，不累死它才怪呢，o(╯□╰)o
